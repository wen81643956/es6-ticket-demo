...
var Base = function () {
  ...
  _createClass(Base, [{
    ...
    value: function setOmit(omit) {
      ...
      (0, _jquery2.default)(_this.omit_el).each(function (index, item) {
        (0, _jquery2.default)(item).text(_this.omit.get(index));
      });
    }
    ...
    value: function changePlayNav(e) {
      ...
      (0, _jquery2.default)('#zx_sm span').html(_this.play_list.get(_this.cur_play).tip);
  ...
  _createClass(Timer, [{
    ...
    value: function countdown(end, update, handle) {
      ...
      if (now - end) {
        ...
        var HOUR_TIMES = 1000 * 60 * 60;
        var MINUTES_TIMES = 1000 * 60;
        var SECOND_TIMES = 1000;
        // 向下取整
        var d = ~~(last_time / DAY_TIMES);
        var h = ~~((last_time - d * DAY_TIMES) / HOUR_TIMES);
        var m = ~~((last_time - d * DAY_TIMES - h * HOUR_TIMES) / MINUTES_TIMES);
